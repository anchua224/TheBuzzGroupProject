...
export default function Comment(props) {
  ...
  const editComment = async () => {
    axios.get(`https://cse216-fl22-team14-new.herokuapp.com/ideas/${props.idea_id}/comment/${props.com_id}?sessionKey=${getSessionKey()}`, {
      // content: content
    })
      .then((response) => {
  ...
  const getComment = async () => {
    axios.get(`https://cse216-fl22-team14-new.herokuapp.com/ideas/${props.idea_id}/comment/${props.com_id}`)
      .then(response => {
  ...
  const handleEdit = async (e) => {
    axios.post(`https://cse216-fl22-team14-new.herokuapp.com/ideas/${props.idea_id}/comment/${props.com_id}?sessionKey=${getSessionKey()}`)
      .catch(error => {
  ...
  return (
    <div className='comment' key={props.com_id}>
      <p>{
        ...
        <button className='user-button'onClick={() => setIsOpen(true)}>Edit</button>
        }</p>
      <EditComment
